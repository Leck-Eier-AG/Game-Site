...
- Current player's actions:
  * "Wuerfeln" button: emit 'game:roll-dice' with keptDice array. Disabled if not your turn or rollsRemaining === 0
  * Clicking scoresheet category: emit 'game:choose-category' with category. Only enabled after at least one roll.
...
Socket events:
- Listen: 'game:state-update' -> update local state
...
interface ScoresheetProps {
  ...
  canScore: boolean // true if it's player's turn and they've rolled
}
```
...
Scoresheet shows potential scores for current dice and allows category selection.
...
- Dice rolling triggers via "Wuerfeln" button -> socket emit
