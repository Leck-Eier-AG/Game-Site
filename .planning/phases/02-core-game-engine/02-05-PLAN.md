...
must_haves:
  truths:
    - "Five 3D dice render in a Canvas with physics simulation"
    - "Dice tumble and bounce realistically when rolled"
    - "Each die displays the correct face value after settling"
    - "Kept dice are visually distinct from rollable dice"
    - "Animation completes within 3 seconds with skip option"
  artifacts:
