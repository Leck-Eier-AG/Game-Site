...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/game/blackjack/handlers.ts"
      ...
      via: "Imports calculatePayouts for bet room settlement"
      pattern: "import.*calculatePayouts.*payout"
    - from: "src/lib/game/blackjack/handlers.ts"
      to: "src/lib/wallet/escrow.ts"
      via: "Uses escrow pattern: LOCKED escrows -> calculatePayouts -> credit winners -> RELEASED"
      pattern: "betEscrow"
---
  ...
  <action>
    1. Create `src/lib/game/blackjack/handlers.ts`:
       ...
       - **Escrow integration (follow existing Kniffel pattern from server.js):**
         ...
         - Import `prisma` (passed as parameter from server.js registration)
         - Import `emitBalanceUpdate` helper (or accept io parameter)
         - On settlement (after dealer plays), for bet rooms:
           a. Query `prisma.betEscrow.findMany({ where: { roomId, status: 'LOCKED' } })` to get total pot
           b. Build rankings from blackjack results (winners position 1, pushes get refund, losers get nothing). For Blackjack: all winners tie at position 1, losers at position 2. Blackjack (3:2) winners get 1.5x their bet from the pot; regular winners get 1x; pushes get exact bet back.
           c. Call `calculatePayouts(totalPot, rankings, room.payoutRatios)` for standard ranked payout, OR implement Blackjack-specific per-player settlement: each player's outcome (win/lose/push/blackjack/surrender) determines their individual payout from their own escrowed bet
           d. Execute in `prisma.$transaction`: credit winners via `tx.wallet.update({ data: { balance: { increment: amount } } })`, create `GAME_WIN` transactions, release escrows via `tx.betEscrow.updateMany({ where: { roomId, status: 'LOCKED' }, data: { status: 'RELEASED', releasedAt: new Date() } })`
           e. Call `emitBalanceUpdate(io, userId, newBalance, change, description)` for each winner
           f. Emit `game:ended` with payoutData array
         - NOTE: Blackjack escrow differs from Kniffel — it's per-player settlement (each player vs dealer), not ranked pot distribution. Consider a per-hand settlement where each player's escrowed bet is individually resolved: win = bet returned + winnings, lose = bet forfeited, push = bet returned, blackjack = bet + 1.5x, surrender = half bet returned.
