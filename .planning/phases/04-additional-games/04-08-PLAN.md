...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/game/roulette/handlers.ts"
      ...
      via: "Imports calculatePayouts for bet room settlement after each spin"
      pattern: "import.*calculatePayouts.*payout"
    - from: "src/lib/game/roulette/handlers.ts"
      to: "src/lib/wallet/escrow.ts"
      via: "Uses escrow pattern: LOCKED escrows -> per-spin settlement -> credit winners -> track running balances"
      pattern: "betEscrow"
---
...
<task type="auto">
  ...
  <action>
    1. Create `src/lib/game/roulette/handlers.ts`:
       ...
       - **Escrow integration (follow existing Kniffel pattern from server.js):**
         - Import `calculatePayouts` from `src/lib/wallet/payout.js`
         - Accept `prisma` as parameter from server.js registration
         - Roulette escrow model: buy-in is escrowed at room join (same as Kniffel: PENDING -> LOCKED at game start). Players bet from their in-game chip pool (tracked in gameState). Per-spin settlement adjusts in-game chip counts. When session ends (host ends or all chips lost), convert remaining chips back to balance.
         - On game start: escrows already locked by server.js `game:start` handler (same pattern as Kniffel)
         - On each spin settlement:
           a. For each player, calculate net result of their bets for that spin (using roulette payout rules from state-machine)
           b. Update player's in-game chip count in gameState
           c. If player's chips hit 0, they're out (can rebuy if settings allow)
         - On game end (host triggers or last player leaves):
           a. Query `prisma.betEscrow.findMany({ where: { roomId, status: 'LOCKED' } })` to get original escrow amounts
           b. For each player, calculate net gain/loss (final chips - initial buy-in chips)
           c. Execute in `prisma.$transaction`: credit players with remaining chips via `tx.wallet.update({ data: { balance: { increment: chipValue } } })`, create `GAME_WIN` or `BET_FORFEIT` transactions, release escrows via `tx.betEscrow.updateMany({ where: { roomId, status: 'LOCKED' }, data: { status: 'RELEASED', releasedAt: new Date() } })`
           d. Call `emitBalanceUpdate(io, userId, newBalance, change, description)` for each player
         - NOTE: Unlike Kniffel (single payout at end), Roulette tracks chips in-game across multiple spins. The escrow covers the initial buy-in, and the final cashout converts remaining chips back to wallet balance.
