...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/game/poker/handlers.ts"
      ...
      via: "Not used directly — Poker uses chip-based cashout, not ranked payout. But may use for tournament-style poker."
      pattern: "calculatePayouts"
    - from: "src/lib/game/poker/handlers.ts"
      to: "src/lib/wallet/transactions.ts"
      via: "Imports getWalletWithUser for rebuy balance checks, creditBalance/debitBalance for chip conversion"
      pattern: "import.*(getWalletWithUser|creditBalance|debitBalance)"
    - from: "src/lib/game/poker/handlers.ts"
      to: "src/lib/wallet/escrow.ts"
      via: "Uses escrow pattern: buy-in escrowed at join, LOCKED at game start, chips tracked in-game, cashout at game end releases escrow"
      pattern: "betEscrow"
---
  ...
  <action>
    1. Create `src/lib/game/poker/handlers.ts`:
       ...
       - `poker:rebuy` — { roomId, amount }
         ...
         c. **Escrow wiring for rebuy:**
            - Import `getWalletWithUser` from `src/lib/wallet/transactions.js`
            - Check wallet balance: `const wallet = await getWalletWithUser(userId); if (wallet.balance < amount) return error`
            - Check wallet not frozen: `if (wallet.frozenAt !== null) return error`
            - Execute in `prisma.$transaction`:
              i. Debit wallet: `tx.wallet.update({ where: { userId }, data: { balance: { decrement: amount } } })`
              ii. Create `BET_PLACED` transaction: `tx.transaction.create({ data: { type: 'BET_PLACED', amount, userId, description: 'Nachkauf: {room.name}' } })`
              iii. Create new escrow: `tx.betEscrow.create({ data: { roomId, userId, amount, status: 'LOCKED' } })` (LOCKED directly since game is in progress)
            - Add rebuy amount to player's chip stack in gameState
            - Track total buy-ins per player in gameState (for net calculation at end)
            - Call `emitBalanceUpdate(io, userId, newBalance, -amount, 'Nachkauf: {room.name}')`

       - `poker:end-game` — { roomId } — Host ends the game session
         a. Only host can trigger
         b. **Escrow wiring for cashout (follow Kniffel end-game pattern from server.js):**
            - Query `prisma.betEscrow.findMany({ where: { roomId, status: 'LOCKED' } })` to get all escrow records
            - For each player, calculate chip value: each player's remaining chips represent their share of the total pot
            - Execute in `prisma.$transaction`:
              i. For each player with chips > 0: `tx.wallet.update({ where: { userId }, data: { balance: { increment: chipValue } } })` and `tx.transaction.create({ data: { type: 'GAME_WIN', amount: chipValue, userId, description: '{room.name} Auszahlung' } })`
              ii. For players with 0 chips: create `BET_FORFEIT` transaction (informational, no balance change since already debited at buy-in)
              iii. Release all escrows: `tx.betEscrow.updateMany({ where: { roomId, status: 'LOCKED' }, data: { status: 'RELEASED', releasedAt: new Date() } })`
            - Call `emitBalanceUpdate(io, userId, newBalance, netChange, description)` for each player
            - Emit `game:ended` with payout data
         c. Transition room to 'ended'
